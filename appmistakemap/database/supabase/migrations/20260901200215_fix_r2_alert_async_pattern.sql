-- O modo síncrono de pg_net (http_collect_response com async := false) trava
-- além do statement_timeout neste projeto, mesmo a resposta HTTP chegando em
-- segundos pelo lado assíncrono (confirmado em teste manual). Troca para o
-- padrão de dois passos: um job dispara a requisição, outro processa a
-- resposta alguns minutos depois. Também corrige a query GraphQL, que faltava
-- selecionar "datetime" como dimensão para poder ordenar por ela.

alter table public.r2_alert_state
  add column if not exists pending_request_id bigint;

drop function if exists public.check_r2_storage_and_alert(text);

select cron.unschedule('check-r2-storage-usage');
...
-- Dispara a requisição a cada 15 min; processa a resposta 2 minutos depois
-- de cada disparo, tempo mais que suficiente para a resposta já ter chegado.
select cron.schedule(
  'request-r2-storage-usage',
  '*/15 * * * *',
  $$select public.request_r2_storage_usage('attempt-submissions');$$
);

select cron.schedule(
  'process-r2-storage-usage',
  '2-59/15 * * * *',
  $$select public.process_r2_storage_usage('attempt-submissions');$$
);
